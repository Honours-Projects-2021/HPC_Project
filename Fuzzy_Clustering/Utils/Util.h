...
vector<double> Util::Fscalar_multiply(double c, vector<double> x,int idx, int len){
...
double Util::Fdistance(vector<double> x1, vector<double> x2,int idx, int len){
    double sum = 0;
    for(int i = idx; i < idx + len; i++){
        sum += pow((x1.at(i) - x2.at(i)),2);
    }
...
vector<double> Util::Fvector_addition(vector<double> x1, vector<double> x2,int idx, int len){
    vector<double> vec;
    for(int i = idx; i < idx + len; i++){
        vec.push_back((Round(x1.at(i) + x2.at(i))));
    }
